...
from button import Button, ScrollArea
...
def handle_events(buttons: list[Button], player: UserPlayer = None, scroll: ScrollArea=None) -> None:
...
def record_score(score):
    ...
    text = score + " " * (6 - len(score)) + f"   {time[0]}   {time[1]}"
    with open("assets/scores.txt") as scores:
        entries = scores.readlines()
    if entries:
        entries.pop(0)
    entries.append(text + "\n")
    entries.sort(reverse=True, key=lambda entry: int(entry.split(" ")[0]))
    with open("assets/scores.txt", "w") as scores:
        scores.write("Score    Date         Time    \n")
...
def show_high_scores():
    ...
    with open("assets/scores.txt") as scores:
        ...
        lines = [line.replace('\n', '')]
        if "No scores" not in line:
            lines.extend(scores.readlines())
...
def game_over(objects):
    record_score(str(objects[0].score))
    buttons = [main_menu_button]
    screen.blit(pygame.image.load("assets/game_background.png"), (0, 0))
    for obj in objects:
        obj.draw(screen)
    while True:
        rendered_text = pygame.font.Font(None, 100).render("YOU LOST!", True, "Red")
        screen.blit(rendered_text, rendered_text.get_rect(center=(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2)))
        handle_events(buttons)
        update_screen()
